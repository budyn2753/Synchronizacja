...
public class DB extends SQLiteOpenHelper {
    ...
    public DB(Context context) {
        super(context, DB_NAZWA, null, DB_VERSION);
    }
...
//
    ...
    public void clearZam() {
        SQLiteDatabase db = this.getReadableDatabase();
        String sql = "DELETE FROM Zamowienie";
        db.execSQL(sql);
    }

    public boolean addSpotkanie(String date, String notatka, int id_klienta) {
        SQLiteDatabase db = this.getReadableDatabase();
        ContentValues contentValues = new ContentValues();

        contentValues.put(KOLUMNA_DATA_SP, date);
        contentValues.put(KOLUMNA_NOTATKA_DO_SPOTKANIA, notatka);
        contentValues.put("Klient_ID", id_klienta);

        db.insert(TABELA_SPOTAKANIA, null, contentValues);
        db.close();
        return true;
    }
    ...
    public ArrayList<String> getClientsNames() {
        ...
        ArrayList<String> listaKliektow = new ArrayList<>();
        String sql = "Select Nazwa, CustomerID_zBazy FROM " + TABELA_KLIENTOW + " Order by " + KOLUMNA_ID_KLIENTA + " ASC;";
        ...
        if (cursor.moveToFirst()) {
            do {
                listaKliektow.add(cursor.getString(1) + " " + cursor.getString(0));
                i++;
            } while (cursor.moveToNext());
        }
        return listaKliektow;
    }
    ...
    public ArrayList<produkty> getProductByID(String ID) {
        ...
        String sql = "Select   SzczegolyZamowienie.Ilosc,  Produkty.Nazwa FROM SzczegolyZamowienie  INNER JOIN Produkty ON SzczegolyZamowienie.ProductID = Produkty.id_baza WHERE SzczegolyZamowienie.IDZamLocal = " + ID + ";";
        ...
        if (cursor.moveToFirst()) {
            do {
                products.add(new produkty(i, cursor.getString(1), Integer.parseInt(cursor.getString(0))));
                i++;
            } while (cursor.moveToNext());
        }
    ...
    public ArrayList<Spotkanie> getSpotkania() {
        ArrayList<Spotkanie> Meetings = new ArrayList<>();

        String sql = "Select * FROM " + TABELA_SPOTAKANIA + " Order by ID_Spotkania ASC;";
        ...
        if (cursor.moveToFirst()) {
            do {
                Meetings.add(new Spotkanie(i, (cursor.getString(1)), cursor.getString(2), (Integer.parseInt(cursor.getString(3)))));
                i++;
            } while (cursor.moveToNext());
        }
        return Meetings;
